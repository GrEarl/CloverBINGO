...
import { useSessionSocket, type ObserverEvent, type ObserverSnapshot } from "../lib/useSessionSocket";
...
export default function ObserverPage() {
  ...
  const recentEvents = useMemo(() => {
    ...
    return view.eventLog.filter((ev) => !ev.type.startsWith("audio.")).slice(-8).reverse();
  }, [view?.eventLog]);

  const tokenQuery = inviteToken ? `?token=${encodeURIComponent(inviteToken)}` : "";
  const debugHref = `/s/${encodeURIComponent(code)}/debug${tokenQuery}`;
  const cardsHref = `/s/${encodeURIComponent(code)}/observer/cards${tokenQuery}`;
  ...
  const playerSummary = useMemo(() => {
    ...
    let disabled = 0;
    for (const p of list) {
      if (p.status === "disabled") disabled += 1;
      else active += 1;
    }
          ...
          <>
            <div className="mt-6 grid gap-4">
              ...
              <div className="grid gap-4 lg:grid-cols-2">
                ...
                <Card>
                  <div className="text-xs text-neutral-500">参加者 / リンク</div>
                  <div className="mt-3 grid gap-2 text-sm">
                    <div>total: <span className="font-mono text-neutral-200">{playerSummary.total}</span></div>
                    <div>active: <span className="font-mono text-emerald-200">{playerSummary.active}</span> / disabled: <span className="font-mono text-rose-300">{playerSummary.disabled}</span></div>
                    <div className="text-xs text-neutral-500">cards: <a className="underline hover:text-neutral-300" href={cardsHref} target="_blank" rel="noreferrer">カード順表示 ({playerCount})</a></div>
                    <div className="text-xs text-neutral-500">debug: <a className="underline hover:text-neutral-300" href={debugHref} target="_blank" rel="noreferrer">/s/{code}/debug</a></div>
                  </div>
                </Card>
              </div>
            </div>
